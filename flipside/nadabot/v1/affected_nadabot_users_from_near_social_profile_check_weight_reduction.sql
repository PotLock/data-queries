-- Affected Users from NEAR Social Check Score Reduction on nadabot v1 https://flipsidecrypto.xyz/wownorth/q/AzdEHFQ_gXTG/affected_user_count-after-weight-change && https://flipsidecrypto.xyz/wownorth/plotlock-plotlock-A36Irq
...
main as (
  ...
  from 
    qmain
  where receiver_id = 'v1.nadabot.near' and weight != 0
),
...
select *, (q.previouse_count - c.current_count) as affected_user_count
from (select count(*) as previouse_count
from human_list_previouse) q, 
(select count(*) as current_count from  human_list_current) c
